...
export class LigueLeadController {
  ...
  @Post('wallets/:walletId/liguelead/sms/filtered') @Roles('ADMIN', 'OPERATIONAL') @HttpCode(202) @Audit({ action: 'LIGUELEAD_SMS_FILTERED_DISPATCH', resourceType: 'Wallet' }) filteredSms(@Param('walletId', ParseUUIDPipe) walletId: string, @Body() dto: SendFilteredLigueLeadSmsDto, @CurrentUser() u: AuthenticatedUser, @Req() req: any) { return this.service.enqueueFilteredSms(walletId, u.accountId, u.id, dto, this.scopes(req)); }
  ...
  @Post('webhooks/liguelead') @Public() @HttpCode(200) webhook(@Body() payload: any, @Headers('authorization-token') authorizationToken?: string, @Headers('x-webhook-token') xWebhookToken?: string, @Headers('authorization') authorization?: string, @Query('token') queryToken?: string) {
