...
export class EmailTemplatesService {
  ...
  async save(walletId: string, accountId: string, input: TemplateInput, id?: string) {
    await this.wallet(walletId, accountId);
    const display = (input.criteria && typeof input.criteria === 'object' ? input.criteria : {}) as Record<string, unknown>;
    if (display.imageUrl && (typeof display.imageUrl !== 'string' || !/^https:\/\//i.test(display.imageUrl))) {
      throw new BadRequestException('A imagem do template deve usar uma URL HTTPS pública');
    }
  ...
  async sendTest(walletId: string, accountId: string, templateId: string, destinationEmail: string) {
    ...
    const display = (template.criteria && typeof template.criteria === 'object' ? template.criteria : {}) as Record<string, unknown>;
    ...
    const header = display.includeCobcomHeader === false ? '' : `<tr><td style="padding:22px 32px;background:#0f4eea"><img src="${appUrl}/cobcom-logo.png" width="52" height="52" alt="CobCom" style="vertical-align:middle;background:#fff;border-radius:10px;padding:4px" /><span style="padding-left:14px;color:#fff;font:700 20px Arial,sans-serif">CobCom</span></td></tr>`;
    const image = typeof display.imageUrl === 'string' ? `<img src="${display.imageUrl}" alt="Imagem do e-mail" style="display:block;max-width:100%;height:auto;margin:0 auto 24px" />` : '';
    const footer = display.includeCobcomFooter === false ? '' : '<tr><td style="padding:20px 32px;background:#f8fafc;color:#667085;font:12px Arial,sans-serif">COBCOM SOLUCOES LTDA · CNPJ 50.703.286/0001-40<br/>bm.cobcomvc.com.br</td></tr>';
    const html = `<!doctype html><html><body style="margin:0;background:#eef3fb"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:32px 16px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#fff;border-radius:16px;overflow:hidden">${header}<tr><td style="padding:36px 32px;color:#1d2939;font:16px Arial,sans-serif;line-height:1.65">${image}${replace(body)}</td></tr>${footer}<tr><td style="padding:16px 32px;color:#667085;font:12px Arial,sans-serif">E-mail de teste — nenhum link é rastreado e nenhum histórico é criado.</td></tr></table></td></tr></table></body></html>`;
  ...
  async send(walletId: string, accountId: string, templateId: string, contractId: string) {
    ...
    const header = display.includeCobcomHeader === false ? '' : `<tr><td style="padding:22px 32px;background:#0f4eea"><table role="presentation" cellspacing="0" cellpadding="0"><tr><td><img src="${logoUrl}" width="52" height="52" alt="CobCom" style="display:block;border:0;object-fit:contain;background:#ffffff;border-radius:10px;padding:4px" /></td><td style="padding-left:14px;color:#ffffff;font:700 20px Arial,sans-serif">CobCom</td></tr></table></td></tr>`;
    const image = typeof display.imageUrl === 'string' ? `<img src="${display.imageUrl}" alt="Imagem do e-mail" style="display:block;max-width:100%;height:auto;margin:0 auto 24px" />` : '';
    const footer = display.includeCobcomFooter === false ? '' : '<tr><td style="padding:20px 32px;background:#f8fafc;color:#667085;font:12px Arial,sans-serif;line-height:1.6;border-top:1px solid #e4e7ec">COBCOM SOLUCOES LTDA · CNPJ 50.703.286/0001-40<br/><a href="https://bm.cobcomvc.com.br/" target="_blank" style="color:#155dfc;text-decoration:none">bm.cobcomvc.com.br</a></td></tr>';
    const html = `<!doctype html><html><body style="margin:0;background:#eef3fb"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#eef3fb"><tr><td align="center" style="padding:32px 16px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 24px rgba(22,47,94,.12)">${header}<tr><td style="padding:36px 32px 20px;color:#1d2939;font:16px Arial,sans-serif;line-height:1.65">${image}${replace(normalizedBody)}</td></tr><tr><td style="padding:8px 32px 36px"><p style="margin:0;color:#667085;font:13px Arial,sans-serif;line-height:1.5">A oferta é individual e o acesso é seguro.</p></td></tr>${footer}</table></td></tr></table><img src="${pixel}" width="1" height="1" alt="" style="display:none"/></body></html>`;
    try {
